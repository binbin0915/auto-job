...
public @interface AutoJob {
    ...
    /**
     * 版本ID，不指定时会使用雪花ID，指定该ID有一定好处，比如在子任务上指定id，可以通过{@link #childTasksId()}关联到该子任务；对于DB任务，该id可以作为一个版本id，同一个版本id里的DB任务为同一个注解扫描任务的不同版本，任务调度的一定是最新版本，DB任务不指定该ID时会基于任务引用的哈希码生成，会有一定的冲突率，因此建议在DB型任务上指定该字段，指定的ID必须是非负数
     */
    long id() default -1L;
