 ...
 */
@Slf4j
public class AutoJobClusterContext extends AbstractLifeCycleHook implements Closeable, WithDaemonThread {
    /**
     * 存储集群节点的容器
     */
    ...
    public boolean updateStatus(ClusterNode node) {
        String key = getNodeKey(node);
        if (!StringUtils.isEmpty(key) && clusterNodeContainer.containsKey(key) && clusterNodes.stream().noneMatch(item -> key.equals(getNodeKey(item)))) {
    ...
    public Iterator<ClusterNode> iterator() {
        return clusterNodeContainer.values().iterator();
    }

    public int getOfflineCount() {
        return (int) getAllNodes().stream().filter(item -> item.getIsOnline() != null && !item.getIsOnline()).count();
    }
    ...
    public void beforeClose(Object... params) throws IOException {
        ...
        for (Map.Entry<String, ClusterNode> entry : clusterNodeContainer.entrySet()) {
            ...
            AutoJobClusterClient client = new AutoJobClusterClient(entry.getValue(), clusterConfig);
            client.close();
        }
    }
    ...
    public void startWork() {
        ...
        Runnable runnable = () -> {
            ...
            if (isInProtectedMode() && !flag.get()) {
                log.warn("已进入保护模式");
                AlertEventHandlerDelegate.getInstance().doHandle(AlertEventFactory.newClusterOpenProtectedModelAlertEvent());
                flag.set(true);
            }
            if (!isInProtectedMode() && flag.get()) {
                log.warn("已退出保护模式");
                AlertEventHandlerDelegate.getInstance().doHandle(AlertEventFactory.newClusterCloseProtectedModeEvent());
                flag.set(false);
            }
        };
        clusterDaemonThread.EFixedRateTask(runnable, 1, 1, TimeUnit.MILLISECONDS);
