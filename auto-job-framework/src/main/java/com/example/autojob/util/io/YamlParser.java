package com.example.autojob.util.io;
...
public class YamlParser {
    ...
    public static Map<String, Object> classPathYaml2FlattenedMap(String yamlClassPath) {
        Yaml yaml = createYaml();
        Map<String, Object> map = new HashMap<>();
        for (Object object : yaml.loadAll(getClassPathResourceInputStream(yamlClassPath))) {
            if (object != null) {
                map = asMap(object);
                map = getFlattenedMap(map);
            }
    ...
    private static InputStream getClassPathResourceInputStream(String fileName) {
        return YamlParser.class.getClassLoader().getResourceAsStream(fileName);
    }
