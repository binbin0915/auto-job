package com.example.autojob.util.cron.util;

import com.example.autojob.util.convert.DateUtils;
import com.example.autojob.util.cron.pojo.CronField;
import com.example.autojob.util.cron.pojo.CronPosition;
import com.example.autojob.util.cron.pojo.DayOfYear;
import com.example.autojob.util.cron.pojo.TimeOfDay;
...
public class CronUtil {
    ...
    public static List<Date> nextCount(int count, String cron, Date start) {
        Date last = start;
        List<Date> dates = new ArrayList<>();
        for (int i = 0; i < count; i++) {
            Date next = next(cron, last);
            last = next;
            dates.add(next);
        }
        return dates;
    ...
    private static DayOfYear findMinDayOfYear(DayOfYear dayOfYearNow, List<Integer> listDay, List<Integer> listMonth, CronField fieldWeek) {
        for (Integer month : listMonth) {
            for (Integer day : listDay) {
                ...
                //大于等于现在的并且满足星期的
                if (dayOfYear.compareTo(dayOfYearNow) >= 0 && satisfy(dayOfYear.week(), fieldWeek)) {
                    return dayOfYear;
                }
            }
        }
    ...
    public static void main(String[] args) {
        String dateString = "2022-08-21 16:30:26";
        try {
            System.out.println(CronUtil.next("5/7 * * * * ?", DateUtils.parseDate(dateString, "yyyy-MM-dd HH:mm:ss")));
